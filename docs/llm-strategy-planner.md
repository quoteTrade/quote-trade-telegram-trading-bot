...
- API keys can be read from environment variables or stored locally in `.quote-trade/llm-config.json`; environment variables are preferred.

## Providers and default free fallback

Supported presets: OpenAI/ChatGPT, Anthropic/Claude, xAI/Grok, OVHcloud AI Endpoints, Google Gemini, OpenRouter, GroqCloud, Hugging Face Inference Providers, Pollinations, and custom OpenAI-compatible gateways.
