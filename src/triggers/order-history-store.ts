...
function normalizeOrder(update: any): OrderHistoryEntry | undefined {
  ...
  return {
    ...
    orderType:
      update.orderType != null ? String(update.orderType) : update.type != null ? String(update.type) : undefined,
    execType: update.execType != null ? String(update.execType) : undefined,
    ordStatus:
      update.ordStatus != null ? String(update.ordStatus) : update.status != null ? String(update.status) : undefined,
...
export class OrderHistoryStore {
  ...
  private reduceOrderTrade(orders: OrderHistoryEntry[]): OrderHistoryEntry[] {
    const reduceOrders = orders.reduce<OrderHistoryEntry[]>((array, current) => {
      ...
      // Fold the previous entry for this order id into the current one. The accumulator is mutated rather
      // than rebuilt because `array.pop()` already mutated it, and rebuilding made this reducer O(n^2).
      current.children = [array[length - 1], ...(array[length - 1].children || [])];
      array.pop();
      array.push(current);
      return array;
    }, []);
  ...
  private orderTradeGroupBy(orders: OrderHistoryEntry[]): {
    ...
    const canceledOrders = orders.filter((order) => {
      ...
      return (
        ["4", "8"].includes(execType) || ["4"].includes(ordStatus) || (["B", "C"].includes(execType) && cumQty === 0)
      );
    });
